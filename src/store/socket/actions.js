...
import {
  ...
  UPDATE_CATEGORY,
  SPLICE_CATEGORY,
  APPEND_CATEGORY_MAPPING,
  SPLICE_CATEGORY_MAPPING,
...
export default {
  [INITIALIZE]({ commit }) {
    ...
    socket.on(`${SERVER}:${CATEGORY_MAPPING_CREATED}`, (data) => {
      commit(`${USER}/${APPEND_CATEGORY_MAPPING}`, data, { root: true })
    });
    socket.on(`${SERVER}:${CATEGORY_MAPPING_DELETED}`, (data) => {
      commit(`${USER}/${SPLICE_CATEGORY_MAPPING}`, data, { root: true })
    });
  },
